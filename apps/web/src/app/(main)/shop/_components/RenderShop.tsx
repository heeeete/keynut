...
const RecentSearch = React.memo(
  ({
    ...
    const handleRecentSearch = (search: string) => {
      ...
      const newRecentSearches = [search, ...recentSearches.filter((item) => item !== search)].slice(
        0,
        5,
      );
    ...
    const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (tempSearchText.trim() !== '') {
        const newRecentSearches = [
          tempSearchText,
          ...recentSearches.filter((item) => item !== tempSearchText),
        ].slice(0, 5);
        setRecentSearches(newRecentSearches);
        localStorage.setItem('recentSearches', JSON.stringify(newRecentSearches));
      }
                  ...
                  <ul className="">
                    {categories.map((category) => (
                      <li key={category.id} className="">
                        <p className="px-3 text-sm font-medium max-[960px]:px-6 max-md:px-3">
                          {category.option}
                        </p>
