...
const DNDImages = React.memo(
  ({ uploadImages, setUploadImages, setDeleteImages }: EditUploadImagesHookProps) => {
          ...
          <Draggable key={idx} draggableId={`draggable-${idx}`} index={idx}>
            {(provided) => (
              ...
              >
                <Image
                  src={
                    !url.startsWith('blob') ? `${process.env.NEXT_PUBLIC_IMAGE_DOMAIN}/${url}` : url
                  }
                  fill
                  alt={`item-${idx}`}
