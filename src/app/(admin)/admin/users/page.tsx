...
const Taskbar = ({ data, page, selectedUsers, setIsLoading, dataRefetch }: TaskbarProps) => {
  const handleOtherProviderWithdrawal = async (_id: string) => {
    try {
  ...
  const onClickWithdrawal = async () => {
    ...
    try {
      for (let key of selectedUsersKeys) {
        const { _id, provider, providerAccountId }: { _id: string; provider: string; providerAccountId: string } =
          selectedUsers[key];
