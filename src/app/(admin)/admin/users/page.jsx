...
const Taskbar = ({ data, page, selectedUsers, setIsLoading, dataRefetch }) => {
  const onClickWithdrawal = async () => {
    ...
    try {
      for (let key of selectedUsersKeys) {
        const { _id, access_token, provider } = selectedUsers[key];

        if (provider === 'kakao') {
          await handleKakaoWithdrawal(_id, access_token);
  ...
  const handleKakaoWithdrawal = (_id, access_token) => {
    return new Promise((resolve, reject) => {
      ...
      Kakao.API.request({
        ...
        .then(async () => {
          ...
          if (!res.ok) reject(new Error('Kakao user deletion error'));
          resolve();
        })
        .catch(error => {
          reject(error);
                ...
                <td className="border-b">
                  <div className="flex justify-center">
                    <input
                      ...
                      onChange={() => handleSelectUser(idx, user._id, user.access_token, user.provider)}
                    />
                  </div>
                </td>
                <td className="border-b">
