...
const deleteViewProduct = (id, setRecentView) => {
  const prev = JSON.parse(localStorage.getItem('recentView') || '[]');

  const updated = prev.filter(data => data._id !== id);

  localStorage.setItem('recentView', JSON.stringify(updated));
  setRecentView(updated);
};

const RecentViewImage = ({ images }) => {
  return (
    <div className="w-16 aspect-square relative">
      <Image
        className="object-cover"
        src={images.length ? `${process.env.NEXT_PUBLIC_IMAGE_DOMAIN}/${images[0].name}` : '/noImage.svg'}
        alt="이미지"
        fill
        sizes="200px"
      />
...
const RecentViewTitleAndPrice = ({ product, setRecentView }) => {
  return (
...
const RecentViewProduct = ({ product, setRecentView }) => {
  return (
...
const RecentViewProducts = () => {
  const { recentViewChange, setRecentViewChange } = useContext(RecentViewContext);
